	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.OPTIONS, "/**").permitAll()
				.antMatchers("/home", "/login", "/token", "/login/v1", "/register", "/upload", "/skills/*",
						...
						"/applications/jobs", "/applications/jobs/{id}/joboffer", "/applications/jobs/user/{userId}", "/applications/jobs/bonjour",

						"/users/**", "/users/{id}","/image/{id:.+}", "/simulations/{id}/answer") // les pages/requêtes /home, /login et /token sont accessibles sans
