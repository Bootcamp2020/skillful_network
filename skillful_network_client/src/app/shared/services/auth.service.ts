...
export class AuthService {
  ...
  login(credentials): Promise<JwtResponse> {
    ...
    return this.api.post({endpoint: '/authentication/login', data: {password: credentials.password, email: credentials.emailLogin}});
}
  ...
  register(credentials): Promise<any> {
    ...
    return this.api.post({ endpoint: '/authentication/register', data: { email: credentials.email, role: credentials.role} });
}
